...
using AcademiaDoZe.Infraestrutura.Tests;

namespace AcademiaDoZe.Infraestrutura.Tests;

public class MatriculaInfraestruturaTests : TestBase
{
    [Fact]
    public async Task Matricula_Adicionar()
    {
        ...
        var matricula = Matricula.Criar(
            1,
            aluno,
            EnumMatriculaPlano.Semestral,
            DateOnly.FromDateTime(DateTime.Today),
            DateOnly.FromDateTime(DateTime.Today.AddMonths(6)),
            "Emagrecer",
            EnumMatriculaRestricoes.Alergias,
            arquivo,
            "Sem observações"
        );
    ...
    public async Task Matricula_ObterPorAluno_Atualizar()
    {
        ...
        Assert.Equal(EnumMatriculaPlano.Anual, resultado.Plano);
    }
